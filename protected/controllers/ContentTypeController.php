...
class ContentTypeController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['ContentType']))
		{
			...
			$model->icon = $fileName;
			$_POST['Content']['icon'] = $model->icon;
			
			$model->attributes=$_POST['ContentType'];
			
			if($model->save()){
				if($uploadedFile !=''){
					$uploadedFile->saveAs(Yii::app()->basePath.'/../images/contentType/'.$fileName);
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['ContentType']))
		{
			...
			if($model->icon =='')
			{
				$model->icon =  rand(0,9999).'-'.$uploadedFile;
				$_POST['ContentType']['icon'] = $model->icon;
			}
			...
			if($model->save())
			{
				if(!empty($uploadedFile))  // check if uploaded file is set or not
                {
                    ...
                    if($uploadedFile != ''){
						$uploadedFile->saveAs(Yii::app()->basePath.'/../images/contentType/'.$model->icon);
	 ...
	 * @param ContentType $model the model to be validated
	 */
